"""Phase 3: MCP Client — Agent Workflows

Builds on Phase 2 by adding automated workflows:
- Python drives the process, calling MCP tools for data gathering
- Claude is invoked only when reasoning is needed
- The MCP server doesn't know or care if it's interactive or batch
...
Usage:
    ...
    python client.py --workflow drug_utilization
    python client.py --workflow cohort_comparison
...
async def run_test_queries(
    ...
    print_header("Phase 3: Agent Workflows — Running Test Queries")
...
async def interactive_mode(
    ...
    """Interactive chat loop with prompt and workflow support."""
    print_header("Phase 3: Agent Workflows — Interactive Mode")
    ...
    while True:
        ...
        if user_input.startswith("/workflow "):
            ...
            if wf_name in WORKFLOWS:
                ...
                result = await WORKFLOWS[wf_name]["function"](session, anthropic_client)
                print(f"\n{result}")
            else:
...
async def main():
    parser = argparse.ArgumentParser(
        description="Phase 3: Agent Workflows clinical intelligence tool"
    )
    ...
    async with stdio_client(server_params, errlog=devnull) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            if args.workflow:
                ...
                result = await wf["function"](session, anthropic_client)
                print(f"\n{result}")
            elif args.prompt:
