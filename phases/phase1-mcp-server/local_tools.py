"""Phase 1: Local tools — stay in the app, not on the MCP server.

These tools are app-specific:
- format_report: output formatting (no data access, no other app reuses this)
- calculate_risk_score: pure computation on already-retrieved data

They demonstrate what DOESN'T need MCP. The LLM calls them the same way
it calls MCP tools — it can't tell the difference.
"""
...
def calculate_risk_score(drug_count: int, interaction_count: int, has_allergies: bool) -> str:
    ...
    Pure computation on already-retrieved data — no data access needed,
    so it stays local.
    """
    score = 0
    score += drug_count * 10
...
# Tool schemas for Claude (Anthropic SDK format)
LOCAL_TOOLS = [
    {
        "name": "format_report",
